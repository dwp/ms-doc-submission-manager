...
@Tag("unit")
class MongoEventListenerSubmissionTest {
  ...
  void listener_updates_channel_for_application() {
    ...
    var event = new BeforeConvertEvent<>(submission, COLLECTION);
    var eventListener = new MongoEventListenerSubmission(watcherConfigProperties);

    eventListener.onBeforeConvert(event);

    verify(watcherConfigProperties, times(1)).setChangeStreamChannel(submission, COLLECTION);
  }
}
